...
def stage_linux_kernel(force_download=False):
    ...
    archive = CACHE_DIR / f"linux-{meta['version']}.tar.gz"
...
def stage_linux_drivers(force_download=False):
    ...
    if downloaded and archive.exists():
        log(f"Extracting complete driver catalog from {archive.name}...", "STEP")
        try:
            driver_categories = (
                "drivers/net",
                "drivers/block",
            ...
            with tarfile.open(archive, "r:*") as tar:
                for member in tar.getmembers():
                    if any(cat in member.name for cat in driver_categories):
                        tar.extract(member, path=src_target)
                    elif "include/linux" in member.name:
                        tar.extract(member, path=inc_target)
            log("Extracted comprehensive Linux driver catalog into driver server.", "SUCCESS")
